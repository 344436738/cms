...
public class ContentsMobileController extends ContentsMobilebaseController {
	...
	public ModelAndView table(ContentsMobileEntity contentsMobile , String publishedStart, String publishedEnd, HttpServletRequest request) {
		...
		if(com.leimingtech.common.utils.StringUtils.isEmpty(mobileChannelId)) {
			mobileChannelId = "-1";
		}
		...
		// 获取内容列表
		int pageSize = com.leimingtech.common.utils.StringUtils.isEmpty(request.getParameter("pageSize")) ? 15 : Integer.valueOf(request.getParameter("pageSize"));
		int pageNo = com.leimingtech.common.utils.StringUtils.isEmpty(request.getParameter("pageNo")) ? 1 : Integer.valueOf(request.getParameter("pageNo"));
	...
	public ModelAndView contentsTree(HttpServletRequest requset) {
		...
		if(user.getAuthentication()== PersonnelStatusConstant.EDITORIAL_STAFF || user.getAuthentication()== PersonnelStatusConstant.ORDINARY_PERSON){
			sortChildCatalog(list);
		}
		Map<String , Object> m = new HashMap<String , Object>();
		m.put("list" , list);
	...
	public void sortChildCatalog(List<MobileChannelEntity> list) {
		if (list != null && list.size() > 0) {
			List<String> pcCatalog=UserUtils.getMobileChannelIdList();
			...
			for (int i = list.size() - 1; i >= 0; i--) {
				if (pcCatalog.contains(list.get(i).getId().toString())) {
					List<MobileChannelEntity> childlist = mobileChannelService.getListByPid(list
							.get(i).getId());
	...
	public ModelAndView showDiv(HttpServletRequest request) {
		...
		if(!checked.equals("")) {
			...
			for(int i = 0 ; i < checkArray.length ; i++) {
				if(com.leimingtech.common.utils.StringUtils.isNotEmpty(checkArray[i])) {
	...
	public ModelAndView correlationDialog(ContentsMobileEntity contentsMobile , String title , String classify1 , HttpServletRequest request) {
		...
		String str = "";
		if(com.leimingtech.common.utils.StringUtils.isNotEmpty(seek)) {
			try {
		...
		// 获取内容列表
		int pageSize = com.leimingtech.common.utils.StringUtils.isEmpty(request.getParameter("pageSize")) ? 10 : Integer.valueOf(request.getParameter("pageSize"));
		int pageNo = com.leimingtech.common.utils.StringUtils.isEmpty(request.getParameter("pageNo")) ? 1 : Integer.valueOf(request.getParameter("pageNo"));
		...
		for(RelateContentMobileEntity relateContent : relateList) {
			...
			if(com.leimingtech.common.utils.StringUtils.isNotEmpty(part)) {
				sql += " and con.id <>" + part;
			}
		}
	...
	public ModelAndView chooseContent(ContentsEntity contents , HttpServletRequest request) {
		...
		String contentId = "";
		if(com.leimingtech.common.utils.StringUtils.isNotEmpty(simplespecial)){
		...
		for(int i=0;i<contentsId.length;i++){
			if(com.leimingtech.common.utils.StringUtils.isNotEmpty(contentsId[i])){
		...
		if(StringUtils.isNotEmpty(mobileId)) {
			...
			for(int i=0;i<catids.length;i++){
				if(com.leimingtech.common.utils.StringUtils.isNotEmpty(catids[i])){
		...
		if(StringUtils.isNotEmpty(simplespecial)&& com.leimingtech.common.utils.StringUtils.isEmpty(contentId)){
			cq.eq("id", "0");
		}
		//判断选取内容为已发的内容
	...
	public String checkContent(HttpServletRequest request) {
		...
		if(com.leimingtech.common.utils.StringUtils.isNotEmpty(contetsId)) {
	...
	public String savePushMobile(String contentId , String mobileChannelId , String classify,String specialids , HttpServletRequest request) {
		JSONObject j = new JSONObject();
		if(com.leimingtech.common.utils.StringUtils.isNotEmpty(mobileChannelId)){
		...
		if(com.leimingtech.common.utils.StringUtils.isNotEmpty(specialids)){
	...
	public ModelAndView setPushParam(HttpServletRequest request) {
		...
		String id = request.getParameter("id");
		String int_id = com.leimingtech.common.utils.StringUtils.isEmpty(com.leimingtech.common.utils.StringUtils.trim(id)) ? 0+"" : id ;
	...
	public JSONObject generateFilter(HttpServletRequest request,HttpServletResponse response) {
		...
		while (iter.hasNext()) {
			...
			if(!com.leimingtech.common.utils.StringUtils.isEmpty(value.toString())) {
	...
	public String sendPush(HttpServletRequest request) {
		...
		String contentJson="";
		if (com.leimingtech.common.utils.StringUtils.isNotEmpty(contentid) && com.leimingtech.common.utils.StringUtils.isNumeric(contentid)) {
...
//				contentJson = "{'id':'"+content.getId()+"','invitationCount':'"+invitationMobileList.size()+"','listUrl':'/mobile/mobile_447.shtml'}";
		...
		try {
			...
			while (paramNames.hasMoreElements()) {
				...
				if(!notParamKey.contains(key)) {
					if(!key.equals("extra")) {
						...
						if(!com.leimingtech.common.utils.StringUtils.isEmpty(value)) {
			...
			if(!com.leimingtech.common.utils.StringUtils.isEmpty(pushToAndroid) && !com.leimingtech.common.utils.StringUtils.isEmpty(pushToIOS)) {
				boolean sendToAndroid = true , sendToIOS = true;
				String realUrl = request.getParameter("url");
				if(com.leimingtech.common.utils.StringUtils.isEmpty(realUrl)) {
