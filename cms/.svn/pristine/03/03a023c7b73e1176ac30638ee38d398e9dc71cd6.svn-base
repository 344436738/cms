...
public class PathFormat {
	...
	public static String parse ( String input ,String isMember) {
		...
		while ( matcher.find() ) {
			...
			matcher.appendReplacement(sb, PathFormat.getString( matcher.group( 1 ) ,isMember) );
	...
	public static String parse ( String input, String filename ,String isMember) {
		...
		while ( matcher.find() ) {
			...
			if ( matchStr.indexOf( "filename" ) != -1 ) {
				...
				matcher.appendReplacement(sb, PathFormat.getString( matchStr ,isMember) );
			}
	...
	private static String getString ( String pattern ,String isMember) {
		...
		} else if(pattern.indexOf( PathFormat.SITEPATH ) != -1){
			return PathFormat.getSitePath(isMember);
	...
	private static String getSitePath (String isMember) {
		String sitePath = "";
		if(StringUtils.isNotEmpty(isMember)){
			sitePath = MemberUtils.getSite().getSitePath();
		}else{
			sitePath = UserUtils.getSite().getSitePath();
		}
		return sitePath;
	}
