...
import com.leimingtech.base.entity.temp.WaterMarkEntity;
...
import com.leimingtech.base.entity.temp.VideoSourcesEntity;
...
public class UploadFileController {
	...
	public JSONObject wordUpload(MultipartHttpServletRequest request, HttpServletResponse response) {
		...
		if (com.leimingtech.common.utils.StringUtils.isNotEmpty(path)) {
			// 自定义文件路径 File.separatorChar
			customPath = "/upload/" + path + "/";
		}
		...
		if (com.leimingtech.common.utils.StringUtils.isNotEmpty(staticFilesPath)) {
			...
			savePath = syspath + com.leimingtech.core.common.CmsConstants.SITE_STORAGE_PATH + "/" + site.getSitePath() + customPath;// 站点模板资源路径
		}
		...
		// 2. getUser each file
		while (itr.hasNext()) {
			// 2.1 getUser next MultipartFile
			mpf = request.getFile(itr.next());
			// 2.2 create new fileMeta
			json.put("fileName", mpf.getOriginalFilename());
			json.put("fileSize", FileUtil.FormetFileSize(mpf.getSize()));
