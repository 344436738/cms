...
public class TSDepartChannelServiceImpl implements TSDepartChannelServiceI {
	...
	@Override
	public List<String> getContentCatIdsByDeparts(List<String> departs) {
		if (departs == null || departs.size() == 0) {
			return null;
		}
		StringBuilder sql = new StringBuilder();
		sql.append(" select channelId from ");
		sql.append(" cms_depart_channel where ");
		List<Object> params = new ArrayList<>();
		if (departs.size() == 1) {
			sql.append(" departId = ? ");
			params.add(departs.get(0));
		} else {
			String[] index = new String[departs.size()];
			for (int i = 0, len = departs.size(); i < len; i++) {
				index[i] = "?";
			}
			sql.append(" departId in (" + StringUtils.join(index, ",") + ") ");
			params.addAll(departs);
		}
		return commonService.findForList(sql.toString(), String.class, params.toArray());
	}
}