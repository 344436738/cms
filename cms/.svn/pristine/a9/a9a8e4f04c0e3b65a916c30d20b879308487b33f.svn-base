	...
	public JSONObject getcontentsMobileList(String all, String mobileChannelId, int pageSize,int pageNo,
		...
		try{
			if(StringUtils.isNotEmpty(all)){
				...
				//专题下没有内容数据
				if(null!=simpleSpecial&& StringUtils.isEmpty(contentIds)){
					cq.eq("id", "0");
				}
				//头图
				if(isTop!=0){
...
//			}
	...
	public BeanListApi topAndList(ContentsMobileEntity contentsMobile){
		...
		if(StringUtils.isNotEmpty(contentsMobile.getPublished())){
...
//		item.setCommentCount(String.valueOf(commentMobileList.size()));//评论条数
		...
		//内容标记
		if(StringUtils.isNotEmpty(contentsMobile.getContentMark())){
...
//				beanApi.setPageItemsCount(String.valueOf(mobileChannel.getMobileChannels().size()));
	...
	public JSONObject savePushMobile(String contentId, String mobileChannelId,String classify) {
		...
		try {
			...
			//发布静态页
			staticMobileImpl.staticMobileArticle(UserUtils.getSite(), contentsMobile, mobileChannel);
			//生成二维码
	...
	public void encode(String contentId) throws Exception{
		...
		String fileNamePath = CmsConstants.getUploadFilesPath(UserUtils.getSiteId())+"/upload/twocode/mobile/";
	...
	public Map<String,Object> getPageList(String publishedEnd,
		...
		//发布时间
		if(StringUtils.isNotEmpty(publishedStart)&& StringUtils.isNotEmpty(publishedEnd)){
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			try {
