package com.leimingtech.cms.service.impl.statictemplate;
...
public class StaticImpl extends CommonServiceImpl implements IStatic {
	...
	public void staticOneSection(SiteEntity site,SectionEntity section) throws Exception {
		...
		try {
			...
			data.put("site", site);
			data.put("domain", site.getProtocol()+site.getDomain());
			data.put("sectionDataListF", dataList);
			data.put("contextpath", Globals.CONTEXTPATH);
			data.put("frontpath", ResourceUtil.getCMSFrontPath());
			...
			if(!file.exists()){
				com.leimingtech.common.utils.LogUtil.error("区块《" + section.getName() + "》发布失败！ 原因：模板不存在"
						+ templateFilepath);
				return;
			}
			...
			com.leimingtech.common.utils.LogUtil.error(message);
		} catch (IOException e) {
			e.printStackTrace();
			message = "区块《" + section.getName() + "》发布失败！ 原因：" + e.getMessage();
			com.leimingtech.common.utils.LogUtil.error(message);
	...
	private void staticOnlyCatalogList(SiteEntity site, ContentCatEntity catalog, long quickCount, long total) throws Exception {
		...
		String frontpath = ResourceUtil.getCMSFrontPath();
		...
		data.put("contextpath", Globals.CONTEXTPATH);
		data.put("frontpath",frontpath);

		if (com.leimingtech.common.utils.StringUtils.isEmpty(catalog.getListModel())) {
			message = "栏目《" + catalog.getName() + "》发布失败！原因：列表模板未配置！";
			com.leimingtech.common.utils.LogUtil.error(message);

		}

		if (com.leimingtech.common.utils.StringUtils.isEmpty(catalog.getPath())) {
		...
		if (com.leimingtech.common.utils.StringUtils.isNotEmpty(catalog.getIndexTemplate())) {
			isSave = true;
			...
			File file = new File(templateFilepath);
			if(!file.exists()){
				com.leimingtech.common.utils.LogUtil.error("栏目《" + catalog.getName() + "》首页发布失败！原因：模板不存在"
						+ templateFilepath);
				return;
			}
			...
			if(com.leimingtech.common.utils.StringUtils.isNotEmpty(site.getIsSwitch())  && site.getIsSwitch().equals("1")){
				catalog.setUrl(frontpath+"/front/newsListController.do?newsList&catId="+catalog.getId());
			}else{
				catalog.setUrl(catalogPath);
			}
			catalog.setStaticUrl(catalogPath);

			catalog.setDynamicUrl(frontpath+"/front/newsListController.do?newsList&catId="+catalog.getId());
			saveOrUpdate(catalog);
		}
		...
		for (int i = 0; i < quickCount; i++) {
			try {
				...
				if(org.apache.commons.lang3.StringUtils.isNotEmpty(rssTemplate)){
					...
					if(!file.exists()){
						com.leimingtech.common.utils.LogUtil.error("栏目rss《" + catalog.getName() + "》第" + (i + 1) + "页发布失败！原因：模板不存在"
								+ rssTemplate);
					}else{
				...
				File file = new File(templateFilepath);
				if(!file.exists()){
					com.leimingtech.common.utils.LogUtil.error("栏目《" + catalog.getName() + "》第" + (i + 1) + "页发布失败！原因：模板不存在"
							+ templateFilepath);
					continue;
				}
	...
	private void staticOnlyContent(ContentsEntity content,ContentCatEntity catalog,SiteEntity site) throws Exception {
		...
		String forntpath = ResourceUtil.getCMSFrontPath();
		...
		try {
			...
			fileH5name="h5/"+content.getId().toString();
			data.put("catalogF", catalog);
			data.put("catalogidF", catalog.getId());
			if (catalog.getIsLeaf() == 1) {
				// 单页栏目
			...
			if (com.leimingtech.common.utils.StringUtils.isEmpty(templatePath)) {
			...
			if(!file.exists()){
				com.leimingtech.common.utils.LogUtil.error("文章《" + content.getTitle() + "》发布失败！原因：模板不存在"
						+ templatePath);
				return;
			}
			...
			if(ContentClassify.CONTENT_ARTICLE.equals(content.getClassify())){   //文章模型
				String templateH5Path=CmsConstants.getSiteTemplatePath(site)+"/wechat/new_detail.html";
				String h5Url=invoke(site, templateH5Path, catalog.getPath(), data, fileH5name, null);
				content.setWechatH5Url(h5Url);
			}

			String staticfilePath = "";
			ArticleEntity art = (ArticleEntity) data.get("articleDataF");
			if (art != null && com.leimingtech.common.utils.StringUtils.isNotEmpty(art.getContent())) {
				...
				if (datapage.length > 1) {
					...
					for (int i = 0; i < datapage.length; i++) {
						...
						com.leimingtech.common.utils.MyBeanUtils.copyBeanNotNull2Bean(art, a);
			...
			if(com.leimingtech.common.utils.StringUtils.isNotEmpty(site.getIsSwitch())  && site.getIsSwitch().equals("1")){
				content.setUrl(forntpath+"/front/newsDeatilController.do?newsDeatil&contentId="+content.getId());
			}else{
			...
			if(com.leimingtech.common.utils.StringUtils.isEmpty(content.getPublishedby())){
			...
			if (catalog.getIsLeaf() == 1) {
				...
				if(com.leimingtech.common.utils.StringUtils.isNotEmpty(site.getIsSwitch())  && site.getIsSwitch().equals("1")){
					catalog.setUrl(forntpath+"/front/newsDeatilController.do?newsDeatil&contentId="+content.getId());
				}else{
	...
	private String generateFile(SiteEntity site,String templateFilepath, Map<String, Object> data, String filePath, String filename,String Suffix) throws Exception {
		...
		// 文件最终生成路径
		if (com.leimingtech.common.utils.StringUtils.isNotEmpty(filePath)) {
		...
		if(StringUtils.isNotEmpty(domainalias)){
			...
			regStr = replaceHtmlTag(ftl, "#include", "<!--#include virtual=\"" +"/"+domainalias+CmsConstants.INCLUDEPATH + "", Suffix
					+ "\"-->");
		}else{
			regStr = replaceHtmlTag(ftl, "#include", "<!--#include virtual=\"" +site.getProtocol()+site.getDomain()+CmsConstants.INCLUDEPATH + "", Suffix
					+ "\"-->");
		}
		...
		String fileDir = com.leimingtech.common.utils.StringUtils.substringBeforeLast(fileNamePath, separator);
	...
	public void staticContentActOnPreview(final ContentsEntity content) {
		com.leimingtech.common.ContextHolderUtils.getRequest().setAttribute("static2Html",true);
		if (content != null) {
			String frontpath = ResourceUtil.getCMSFrontPath();
			String filename = "";
			...
			try {
				...
				if (com.leimingtech.common.utils.StringUtils.isEmpty(templateFilepath)) {
				...
				File file = new File(templateFilepath);
				if(!file.exists()){
					com.leimingtech.common.utils.LogUtil.error("文章《" + content.getTitle() + "》发布失败！原因：模板不存在" + templateFilepath);
					return;
				}
				...
				if(com.leimingtech.common.utils.StringUtils.isNotEmpty(site.getIsSwitch())  && site.getIsSwitch().equals("1")){
					content.setUrl(frontpath+"/front/newsDeatilController.do?newsDeatil&contentId="+content.getId());
				}else{
					content.setUrl(staticfilePath);
				}
				content.setStaticUrl(staticfilePath);
...
//				LogUtil.info(message);
				...
				com.leimingtech.common.utils.LogUtil.error(message,e);
			}
		}
	...
	private String invoteThread(SiteEntity site,String filename, Map<String, Object> data, String templateFilepath, String filePath, String returnPath,
		...
		try {
			...
			if(StringUtils.isNotEmpty(domainalias)){
				...
				regStr = replaceHtmlTag(ftl, "#include", "<!--#include virtual=\"" +"/"+domainalias+CmsConstants.INCLUDEPATH + "", staticSuffix
						+ "\"-->");
			}else{
				regStr = replaceHtmlTag(ftl, "#include", "<!--#include virtual=\"" +site.getProtocol()+site.getDomain()+CmsConstants.INCLUDEPATH + "", staticSuffix
						+ "\"-->");
			}
			File tempFile=FileUtils.getTmpFile();
...
//				LogUtil.info(message);
		...
		private String invoteThread(String filename, Map<String, Object> data, String templateFilepath, String filePath, String returnPath,
			...
			try {
				...
				if (com.leimingtech.common.utils.StringUtils.isEmpty(filename)) {
					filename = FileUtils.getFilePrefix(file.getName()) + "_0";
				}
	 ...
	 */
	@Override
	public void getFileHtml(SiteEntity site, String templateFilepath, Map<String, Object> data,HttpServletResponse response) {
		...
		try {
			...
			cfg.setDirectoryForTemplateLoading(new File(CmsConstants.getSiteTemplatePath(site)));
			...
			pw.print(org.apache.commons.io.FileUtils.readFileToString(tempFile, "utf-8"));
			pw.flush();
			pw.close();
			tempFile.delete();
		}catch (IOException e) {
