...
function treeMenu(urlStr){
	$.ajax({
		...
		success:function(msg){
			...
			$(".nav a[onclick*='treeMenu('"+urlStr+"']").parents("li").addClass("active").filter(":last").find("span:eq(0)").removeClass("arrow").addClass("selected");
			$('#leftmenu').empty();
			$('#leftmenu').append(msg);
			$(".nav li[class*='active']").removeClass("active").find("span").removeClass("selected");
			$("a[onclick*='"+urlStr+"']").parent().addClass("active").find("span").addClass("selected");
			App.fixContentHeight();
		},
		error:function(){
			lmAlert("load page error, page url is " + urlStr);
