...
public class ReplyFrontServiceImpl implements ReplyFrontServiceI {
	 ...
	 * @return
	 */
	public String save(ReplyEntity reply) {
		return (String) commonService.save(reply);
	}
	 ...
	 */
	@Override
	public ReplyEntity getEntity(String id) {
    ...
    public Integer addReplySupport(String replyId) {
        String sql = "update cms_reply set support_count = support_count+1 where id = '"
                + replyId + "'";
    ...
    public Integer deleteReplySupport(String replyId) {
        String sql = "update cms_reply set oppose_count = oppose_count+1 where id = '"
                + replyId + "'";
        commonService.executeSql(sql);
        ReplyEntity reply = commonService.get(ReplyEntity.class, replyId);
