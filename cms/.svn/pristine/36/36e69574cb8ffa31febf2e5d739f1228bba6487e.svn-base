		...
		for (int i = 0; i < functionList.size(); i++) {
			...
			if (function != null) {
				if (function.getTSFunctions() == null || function.getTSFunctions().size() == 0) {
					...
					if(user.getAuthentication() == Globals.ADMINISTRATOR_SITE || Globals.ORDINARY_PERSON == user.getAuthentication()){
						childFunList = function.getTSFunListControl();
					}else{
						childFunList = function.getTSFunctions();
					}
					getHorezantalNode(sb, function,childFunList);
					index++;
				}
//				if (index == 8)
//					break;
...
/**
	...
	private static void getHorezantalNode(StringBuilder sb, TSFunction function,List<TSFunction> childFunList) {
		// ----------------一级节点开始----------------
		sb.append("<li >\n");
				...
				if(privurl != null){
					if (privurl.contains(childfuntion.getFunctionUrl())) {
						if("#".equals(childfuntion.getFunctionUrl())){
							Map<String, TSFunction> functionMap = PlatFormUtil.getUserFunction(UserUtils.getSite());
