...
public class ContentTagMng extends CommonServiceImpl implements IContentTagMng {
	...
	public Object getContentByTag(Map params) {
		...
		if(com.leimingtech.common.utils.StringUtils.isEmpty(name) && com.leimingtech.common.utils.StringUtils.isNotEmpty(weight)){
	...
	private void queryAllArticle(CriteriaQuery cq, String name, Boolean page) {
		...
		if (catalog == null) {
			//当栏目不存在查不出数据
			cq.eq("id", "-1");
		}else if(RequestContextHolder.getRequestAttributes()!=null && com.leimingtech.common.ContextHolderUtils.getSession()!=null  && com.leimingtech.common.ContextHolderUtils.getRequest().getAttribute("static2Html")==null){
	...
	private void setOrder(CriteriaQuery cq, String order) {
		if (com.leimingtech.common.utils.StringUtils.isNotEmpty(order)) {
	...
	public String getContentTemplatePath(ContentsEntity article,ContentCatEntity catalog) {
		...
		if (com.leimingtech.common.utils.StringUtils.isEmpty(modelType))
			return null;
	...
	private Map<String, Object> getArticleData(String articleid) {
		...
		String uploadPath = com.leimingtech.common.utils.ResourceUtil.getCMSUploadFilesPath();
	...
	public String replaceWapStr(String str) {
		if (com.leimingtech.common.utils.StringUtils.isNotEmpty(str)) {
	...
	private Map<String, Object> getActivity(String articleid) {
		...
		if (activityList != null && activityList.size() > 0) {
			ActivityEntity activity  = activityList.get(0);
			String activityContent = activity.getActivitycontent();
			activityContent =  StringEscapeUtils.escapeHtml4(activityContent);
			activity.setActivitycontent(activityContent);
			m.put("activityF", activity);
			m.put("sysdate",new Date());
			String sql = "select apc.id  id, apc.optionids  optionids,ap.validation  validation,ap.textsize_limit  textsizeLimit ,"
					+ "ap.selectsize_limit  selectsizeLimit, ap.filesize_limit  filesizeLimit, "
					+ "ap.optional_value  optionalValue,ap.date_type  dateType,apc.optionids  optionids,"
					+ "ap.is_show  isShow, ap.option_name  optionName,apc.is_enableds  isEnableds,apc.is_receptionShow "
					+ " isReceptionshow,apc.is_required  isRequired"
		...
		Map<String, Object> map = ActivityOptionList.getUser(i) ;
		Set<String> set = map.keySet();
		List<String> keyList = new ArrayList<String>();
		keyList.addAll(set) ;
		for (int j = 0; j < keyList.size(); j++) {

		System.out.println("key:" + keyList.getUser(4) + "---Value:" + map.getUser(keyList.getUser(4)));
		}
		}*/
	...
	public Object getContentByTagTop(Map params) {
		...
		if (page && com.leimingtech.common.utils.StringUtils.isNotEmpty(pageindex)
				&& com.leimingtech.common.utils.StringUtils.isNumeric(pageindex)) {
		...
		// String type = (String) params.getUser("type");
		...
		if (!page && contentList != null && !getSelf) {
			...
			if (com.leimingtech.common.utils.StringUtils.isNotEmpty(count) && com.leimingtech.common.utils.StringUtils.isNumeric(count)) {
