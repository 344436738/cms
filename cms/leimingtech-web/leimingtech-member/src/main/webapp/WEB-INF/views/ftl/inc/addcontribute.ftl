		...
		<tr>
				...
				<td><select name="contentCatId" id="contentCatId"
					style="height:30px;">
						...
						<#--<option value="0">新闻</option>-->
							<#list contentcatList as con>




                                <option value="${con.catId}"
									<#if contents.catid?if_exists[0]?has_content>
										<#if con.catId==contents.catid>
                                        selected="selected"
										</#if>
						</#if> >
							${con.catName?if_exists}</option>
						</#list>
			...
			<tr>
				...
				<td>	验证码：<input type="text" name="valicode" id="valicode"
					...
					alt="点击刷新验证码" id="randCodeImage" src="${domain}/${base}/randCodeImage"
				...
				<td class="tc" colspan="4">
					<div class="form-actions" style="text-align: center">
						<input type="button" class="btn btn-primary"
							onclick="subContentform('saveContribute', 'subform');"
							value="保存投稿">
					</div></td>
			</tr>
		</table>
...
<script type="text/javascript"
	src="${domain}/${base}/res/js/jquery-2.1.1.js"></script>
