...
public class JdbcDao extends SimpleJdbcTemplate{
	...
	public List<T> findObjForJdbc(String sql, int page, int rows,Class<T> clazz) {
		...
		for(Map<String,Object> m:mapList){
			try {
				...
				com.leimingtech.common.utils.MyBeanUtils.copyMap2Bean_Nobig(po, m);
				rsList.add(po);
			}  catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static String lmcmsCreatePageSql(String sql, int page, int rows){
		...
		sqlParam[1] = beginNum+"";
		sqlParam[2] = rows+"";
		if(com.leimingtech.common.utils.ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_MYSQL)!=-1){
			sql = MessageFormat.format(MYSQL_SQL, sqlParam);
		}else if(com.leimingtech.common.utils.ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_POSTGRE)!=-1){
			...
			if(com.leimingtech.common.utils.ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_ORACLE)!=-1) {
				sql = MessageFormat.format(ORACLE_SQL, sqlParam);
			} else if(com.leimingtech.common.utils.ResourceUtil.getJdbcUrl().indexOf(DATABSE_TYPE_SQLSERVER)!=-1) {
