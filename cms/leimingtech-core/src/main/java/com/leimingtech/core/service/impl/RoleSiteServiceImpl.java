...
public class RoleSiteServiceImpl implements RoleSiteServiceI {
	...
	public void saveSiteRole(String roleId, String funVal) {
		...
		if (roleSietList.size() == 0) {
			for (int i = 0; i < funValArray.length; i++) {
				...
				if (com.leimingtech.common.utils.StringUtils.isBlank(funValId)) {
					continue;
				}
	...
	public int deleteByRoleId(String roleId) {
		return this.commonService.executeSql(" delete from cms_role_site where role_id = ? ", roleId);
	}
	 ...
	 */
	@Override
	public List<SiteVOSimple> getSiteListByUserId(String userId){
		/*StringBuilder sql = new StringBuilder();
		sql.append(" select s.id,s.site_name,s.short_name from cms_site s ");
		sql.append(" inner join cms_role_site rs on rs.site_id=s.id ");
		sql.append(" where rs.role_id in (select ru.roleid from  cms_role_user ru ");
		sql.append(" where ru.userid = ? ) group by s.id order by is_master desc,createdtime asc ");
		List<SiteVOSimple> list = this.commonService.query(sql.toString(), new SiteVOSimple(), userId);*/

		//站点不和角色关联、根据管理员分配获取
		StringBuilder sql = new StringBuilder();
		sql.append(" select s.id,s.site_name,s.short_name from cms_site s ");
		sql.append(" inner join cms_site_user su on su.site_id=s.id ");
