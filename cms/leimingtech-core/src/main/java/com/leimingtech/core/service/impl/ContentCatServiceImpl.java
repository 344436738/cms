...
public class ContentCatServiceImpl implements ContentCatServiceI {
	 ...
	 */
	@Override
	public JSONArray getzTreeData(String siteId) {
		return getzTreeData(siteId,null);
	}
	...
	public void filterPCCatalogPermission(List<ContentCatVOTreeEntity> list) {
		if (list != null && list.size() > 0) {
			List<String> contentCatIds = UserUtils.getContentCatIdList();
			if (contentCatIds == null || contentCatIds.size() == 0) {
				list.clear();
				return;
			}
			for (int i = list.size() - 1; i >= 0; i--) {
				if (!contentCatIds.contains(list.get(i).getId().toString())) {
					list.remove(i);
				}
			}
		}
     ...
     */
	@Override
	public List<ContentCatVOTreeEntity> getFirstCatList(String siteId) {
		CriteriaQuery cq = new CriteriaQuery(ContentCatEntity.class);
		DetachedCriteria dc = cq.getDetachedCriteria();
		ProjectionList pList = Projections.projectionList();
		pList.add(Projections.property("id").as("id"));
		...
		cq.addOrder("created", SortDirection.desc);
		cq.add();
		List<ContentCatVOTreeEntity> catList = commonService.getListByCriteriaQuery(cq,
				false);
		return  catList;
	}
	 ...
	 */
	@Override
	public Integer checkUniqueCodeIsExist(String catId,String uniqueCode,String siteid) {
		String sql = " select count(*) count from cms_content_cat where unique_code = ? and site_id = ?";
		ArrayList<String> conditions = new ArrayList<>();
		conditions.add(uniqueCode);
		conditions.add(siteid);
		if (StringUtils.isNotEmpty(catId)){
			sql += " and id != ?";
			conditions.add(catId);
		}
		Map<String, Object> result = this.commonService.findOneForJdbc(sql, conditions.toArray());
		int count = oConvertUtils.getInt(result.get("count"), 0);
		return count;
	}
}